...
int zheng(int c)
{
  ...
  if((fp=fopen("D:\\学习\\程序设计\\project\\buying.txt","a+"))==NULL)
     {
...
int tuiyl(int c)
{
	...
	scanf("%s",a);
	if((sp=fopen("D:\\学习\\程序设计\\project\\yinliao.txt","r"))==NULL) 
		{ 
    ...
    fclose(sp);
    if((sp=fopen("D:\\学习\\程序设计\\project\\yinliao.txt","w+"))==NULL) 
		{ 
		printf("file can’t be opened"); 
		exit(0); 
		}
...
int cha(int c)
{
	...
	int number,price[5];
	FILE *fp;
	if((fp=fopen("D:\\学习\\程序设计\\project\\canteen.txt","r"))==NULL)
    {
...
int lu(int c)
{
	...
	/*if((fp=fopen("student.txt","w"))==NULL)
	 ...
	 fprintf(fp,"%s %s %d %d\n",name,sex,age,number);
	 fclose(fp);*/
	 	if((fp=fopen("D:\\学习\\程序设计\\project\\student.txt","a+"))==NULL)
     {
	  puts("cannot be opened");
	  exit(0);
	 }
...
int sttjxf(int c)
{
	...
	FILE *sp;
	if((sp=fopen("D:\\学习\\程序设计\\project\\buying.txt","rb"))==NULL)
    {
	puts("can’t be opened"); 
	exit(0); 
	}
...
struct stud *get_key(struct stud *head,int a[])
{
	int n,i,m,b[5];
...
int main()
{
	...
	if(x==2)
   {
    ...
    printf("紫荆食堂5天内每种菜肴的销售总额分别为A %d元、B %d元、C %d元、D %d元、E %d元\n桃李食堂5天内每种菜肴的销售总额分别为A %d元、F %d元、I %d元、J %d元、K %d元\n玉树食堂5天内每种菜肴的销售总额分别为A %d元、B %d元、L %d元、M %d元、N %d元\n丁香食堂5天内每种菜肴的销售总额分别为D %d元、O %d元、P %d元、Q %d元、R %d元\n芝兰食堂5天内每种菜肴的销售总额分别为B %d元、C %d元、E %d元、X %d元、Y %d元\n",6*add(ZIA),5*add(ZIB),4*add(ZIC),3*add(ZID),1*add(ZIE),6*add(TAOA),5*add(TAOF),4*add(TAOI),3*add(TAOJ),3*add(TAOK),6*add(YUA),5*add(YUB),5*add(YUL),7*add(YUM),9*add(YUN),3*add(DINGD),6*add(DINGO),8*add(DINGP),8*add(DINGQ),6*add(DINGR),5*add(ZHIB),4*add(ZHIC),2*add(ZHIE),10*add(ZHIX),9*add(ZHIY));
	fclose(fp); 	
	}*/
