...
int scrm(int c)		//输出学生信息
{
	...
	printf("姓名  性别  年龄  学号\n");
	FILE *fp;
	if((fp=fopen("D:\\2015011921\\数据文件夹\\student.txt","r"))==NULL)
...
int zheng(int c)		//购买正餐函数
{
	struct ord a;
	FILE  *fp;
  if((fp=fopen("D:\\2015011921\\数据文件夹\\buying.txt","a+"))==NULL)
     {
...
int tuiyl(int c)		//退饮料函数
{
	...
	scanf("%s",a);
	if((sp=fopen("D:\\2015011921\\数据文件夹\\project\\yinliao.txt","r"))==NULL) 
		{ 
    ...
    fclose(sp);
    if((sp=fopen("D:\\2015011921\\数据文件夹\\yinliao.txt","w+"))==NULL) 
		{ 
		printf("file can’t be opened"); 
		exit(0); 
		}
...
int maiyl(int c)	//买饮料函数
{
	...
	if((sp=fopen("D:\\2015011921\\数据文件夹\\yinliao.txt","r"))==NULL) 
	{ 
    	printf("file can’t be opened"); 
	    exit(0); 
	}
...
int cha(int c)		//查询食堂信息函数
{
	...
	int number,price[5];
	FILE *fp;
	if((fp=fopen("D:\\2015011921\\数据文件夹\\canteen.txt","r"))==NULL)
    {
...
int lu(int c)		//录入学生信息函数
{
	...
	printf("请输入学生信息，格式为：姓名/性别/年龄/学号\n");
	 	if((fp=fopen("D:\\2015011921\\数据文件夹\\student.txt","a+"))==NULL)
     {
...
int sttjxf(int c)		//统计食堂销售额函数
{
	...
	FILE *sp;
	if((sp=fopen("D:\\2015011921\\数据文件夹\\buying.txt","rb"))==NULL)
    {
	puts("can’t be opened"); 
	exit(0); 
	}
...
int xstjxf(int c)		//统计学生消费函数
{
	...
	FILE *fp;
	struct ord a;
	if((fp=fopen("D:\\2015011921\\数据文件夹\\canteen.txt","rb"))==NULL)
    {
	puts("can’t be opened"); 
	exit(0); 
	}
	...
	if((fp=fopen("D:\\2015011921\\数据文件夹\\student.txt","rb"))==NULL)
    {
	puts("can’t be opened"); 
	exit(0); 
	}
	...
	if((fp=fopen("D:\\2015011921\\数据文件夹\\buying.txt","rb"))==NULL)
    {
	puts("can’t be opened"); 
	exit(0); 
	}
...
{ 
    ...
    while(i<6)//链表1输入数据
    {  
      if((fp=fopen("D:\\2015011921\\数据文件夹\\xiaoshouzonge.txt","rb"))==NULL)
    	{
		puts("can’t be opened"); 
		exit(0); 
		}
